...
# do_pft <- c("GRA", "CRO", "Forests", "Savannas")
do_pft <- c("Forests")

foreach(namePFT = do_pft) %dopar% {
  ...
  # reduce data to PFT, USA, and predictors
  df <- df_SHAP %>%
    ...
    dplyr::select(
      lon,
      lat, # keep lon/lat to save df train later
      SIF_over_PAR,
      WTD,
      P_over_Rn
      ) %>%   # select predictors
  ...
  if (dorun) {
    ...
    out_train |>
      ggplot(aes(train_y, pred_y_train)) +
      geom_hex(bins = 60)
